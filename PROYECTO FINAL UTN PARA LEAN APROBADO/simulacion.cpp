...
void Simulacion(int puntajeMayor[], string nombreDelMayorPuntaje[])
{
    ...
    while(finaliza == false)
    {
        ...
        rlutil::locate (48,10);
        cout << "INGRESA 6 NUMEROS DE DADOS ENTRE 1 Y 6 " <<endl;
        rlutil::locate (48,12);
        ...
        for (int i=0; i<6 ; i++)
        {
            ...
            if (dados[i] > 6 || dados[i] < 1){
                rlutil::locate (40,i+12);
                cout << "DEBES INGRESAR UN NUMERO ENTRE 1 Y 6, LA APLICACION SE CIERRA" ;
                exit(0);
        }
        }
        while (tirada <=2)
        {
            ...
            for (int j=0; j<6; j++)
            {
                Dados[j] = tirarAEleccion (dados[j], (j + 3) *10,16,1);
            }
/// AL LLAMAR A LA FUNCION PARA TIRAR LOS DADOS LE DAMOS LOS NUMERO QUE ELIGIO EL USUARIO
